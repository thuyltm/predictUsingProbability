...
Hàm kernel có các thuộc tính giống hàm mật độ xác suất (pdf) với một thuộc tính được cộng thêm là hàm kernel bắt buộc phải hàm chẵn (even function), nghĩa là với mọi giá trị x và -x trong miền giá trị f, ta có: f(x) = f(-x). Nói cách khác, hàm chẵn đối xứng qua trục tung y, ví dụ như |x|, $x^2$, cos(x). Với sự thuận tiện trong Toán học, phân phối chuẩn thường được dùng là hàm kernel.\\
Trong thống kê, thuật toán Kernel Desity Estimation (KDE) là phương pháp không tham số (non-parametric method) dự đoán hàm mật độ xác suất pdf của biến ngẫu nhiên liên tục. Phương pháp không tham số nghĩa là không giả định biến ngẫu nhiên x tuân theo bất kỳ phân phối nào bởi vì kích thước dữ liệu quá nhỏ nên định lý trung tâm (central limit theorem) không được sử dụng để giả định phân phối. KDE là phương pháp cơ bản để làm trơn dữ liệu, dựa vào dữ liệu mẫu giới hạn để suy luận tổng thể.\\
Điều quan trọng, tại mỗi điểm dữ liệu, hàm kernel được tạo sao cho mỗi điểm dữ liệu là trung tâm của hàm, điều này bảo đảm hàm kernel đối xứng qua điểm dữ liệu. Sau đó, tại mỗi điểm dữ liệu, thực hiện dự đoán hàm mật độ xác suất pdf sao cho f(x) thỏa mãn tính chất f(x) > 0 và $\int_{-t}^t f(x) = 1$ với x $\in$ [-t,t].\\
...
Tóm lại, những bước tạo thành KDE:\\
\begin{enumerate}
\item Chọn hàm kernel, thường là phân phối chuẩn (Gaussian), phân phối đều (hình chữ nhật hay hình tam giác)
\item Tại mỗi điểm dữ liệu $x_i$, xây dựng hàm kernel thu nhỏ $\frac{1}{h}K[\frac{(x-x_i)}{h}]$, trong đó K là hàm kernel được chọn, tham số h được gọi là bandwidth, hoặc tham số trơn. 
\item Cộng tất cả các hàm kernel thu nhỏ riêng lẻ và chia cho n, điều này thể hiện xác suất của $\frac{1}{n}$ đối với mỗi $x_i$. Đồng thời, điều này đảm bảo rằng tích phân trên miền giá trị của KDE bằng 1.\\  
\[
